...
const MovieDetail = () => {
  ...
  const handleWatch = () => {
    ...
    // isPurchased (not hasValidPurchase): the client-side ppvExpiry check was
    // removed on the branch this merges, so entitlement is the backend's
    // purchases list alone -- there is no local expiry variable left to read.
    if (isFilmmaker || isPurchased || (isSubscribed && subscriptionCovers)) {
